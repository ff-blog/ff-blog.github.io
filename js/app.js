...
window.onload = function() {
    var $body = document.body,
        ...
        $toc_share = document.getElementById("article-toc-share"),
        timer = null;
    ...
    window.addEventListener('scroll', function() {
        var scrollResult = getScrollTop();
        var top = 200; // 用于滚动事件低于该值时恢复
        if ($toc_share){
          var left = $toc_share.offsetLeft;
          var width = $toc_share.offsetWidth;
          if(scrollResult <= top){
            $toc_share.style = "";
          } else {
            $toc_share.style.position = "fixed";
            $toc_share.style.top = "10px";
            $toc_share.style.left = left + "px";
            $toc_share.style.width = width + "px";
            $toc_share.style.marginBottom = "1rem";
          }
        }
        ...
        if($toc){
            ...
            if(scrollResult <= top){
                ...
                $toc.style.top = ($toc_share == null?10:$toc_share.offsetHeight+20) + "px";
